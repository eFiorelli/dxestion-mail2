	...
	try {
		...
		if (storeDB) {
			...
			if (req.files) {
				...
				const updatedStore = await savePromotionImages(store, images);
				if (!updatedStore.ok) {
					return res.status(400).json({
						ok: false,
...
savePromotionImages = async (promotionDB, images) => {
	try {
		for (let i = 0; i < images.length; i++) {
			...
			if (file) {
				...
				// Change filename
				let filename = `${promotionDB._id}-${new Date().getMilliseconds() * Math.random()}.${extension}`;

				let type = '';
				// Use the mv() method to place the file somewhere on your server
				if (images[i].type === `background_${i + 1}`) {
					promotionDB.background_img[i] = filename;
					type = 'background';
				}
				if (images[i].type === 'logo') {
					promotionDB.logo_img = filename;
					type = 'logo';
				}

				await file.mv(`uploads/promotions/${type}/${filename}`);
			}
		}
		return { ok: true, promotionDB };
	} catch (err) {
