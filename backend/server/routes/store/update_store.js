		...
		if (!storeDB) {
			...
			for (let i = 0; i < images.length; i++) {
				...
				if (file) {
					...
					const oldFilenames = [
						storeDB[`background_img_${i + 1}`],
						storeDB.logo_img
					];

					let type = "";
					if (images[i].type === `background_${i + 1}`) {
						if (storeDB[`background_img_${i + 1}`]) {
							deleteStoreFiles("background", oldFilenames[0]);
						}
						storeDB[`background_img_${i + 1}`] = `${filename}`;
						type = "background";
					}
			...
			let bg_img = [];
			bg_img.push(storeDB.background_img_1);
			bg_img.push(storeDB.background_img_2);
			bg_img.push(storeDB.background_img_3);
			bg_img.push(storeDB.background_img_4);
			bg_img.push(storeDB.background_img_5);
